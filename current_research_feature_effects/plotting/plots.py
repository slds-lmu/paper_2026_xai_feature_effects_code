...
def plot_mean_estimation_error(
    estimation_error_data: Dict[float, FeatureEffect],
    feature_names: List[str],
    title: Optional[str] = None,
    sharey: bool = False,
    xscale: Literal["linear", "log"] = "linear",
    yscale: Literal["linear", "log"] = "log",
    plot_rate: bool = False,
    K: int = 100,
    large_font: bool = False,
    save_figs: None | Path = None,
) -> plt.Figure:
    """
    Plot the mean estimation error over features for different sample sizes.

    This function creates a series of line plots, each representing the mean estimation error
    of a feature effect estimate across different sample sizes. The x-axis
    represents the sample sizes, while the y-axis represents the mean estimation error.

    Parameters
    ----------
    estimation_error_data : Dict
        A dictionary containing the estimation error data for different sample sizes
        and features.
    feature_names : List[str]
        A list of feature names to plot.
    title : str, optional
    ...
    plot_rate : bool, optional
        If True, plot the rate 1/n_samples and K/n_samples as dashed and dotted lines
        respectively (default is False).
    K : int, optional
        The constant K to plot the rate K/n_samples as a dotted line if plot_rate is
        True (default is 100).
    large_font : bool, optional
        If True, use larger font sizes (default is False).
    save_figs : None | Path
        Path + filename to save the figure (default is None).
    ...
    for feature, ax in zip(feature_names, axes):
        means = []
        for k in estimation_error_data.keys():
            means.append(np.mean(estimation_error_data[k].features[feature]["effect"]))
        ax.plot(estimation_error_data.keys(), means, marker="+", color=palette[4])  #, label="mean estimation error")
        if plot_rate:
            ax.plot(estimation_error_data.keys(), [1/n for n in estimation_error_data.keys()], "--", label="$1/n$", color="black")
            ax.plot(estimation_error_data.keys(), [K/n for n in estimation_error_data.keys()], ":", label="$K/n$", color="black")
    ...
    if save_figs is not None:
        fig.savefig(save_figs, bbox_inches="tight")
